...
name|this
argument_list|,
literal|"A user must be a member of at least one user group"
argument_list|,
...
argument_list|()
decl_stmt|;
comment|/**              * We update the account in three stages:              *              * 1) General account properties              * 2) Group memebrship              * 3) Optionally set changed password.              *              * The password is always changed last if needed,              * as it means the admin client must reconnect              * if we are changing the logged in users password.              *              * The reconnection is performed by the registered              * DialogCompleteWithResponse handler              */
comment|//1) Update general account properties
...
argument_list|)
expr_stmt|;
comment|//2) Update group membership (has to be modified separately from (1))
name|modifyAccountGroupMembership
argument_list|()
expr_stmt|;
comment|//3) Finally, optionally change the password
if|if
condition|(
name|newPassword
operator|.
...
expr_stmt|;
block|}
comment|/**      * Updates the account with all of the fields from the form      * except the password.      *      * If the password on the form has been changed then it is returned      * as the result of this function      */
specifier|private
name|Optional
argument_list|<
name|String
