...
argument_list|()
expr_stmt|;
comment|//creating a user may have created a private group for that user
block|}
block|}
argument_list|)
expr_stmt|;
...
specifier|private
name|UserManagementService
name|reconnectClientAndUserManager
parameter_list|(
specifier|final
name|String
name|password
...
name|XMLDBException
block|{
comment|//get client to reconnect with edited users new password
specifier|final
name|Properties
name|loginData
init|=
...
argument_list|)
expr_stmt|;
comment|//get reconnected userManagementService
return|return
name|client
operator|.
...
name|println
argument_list|(
literal|"Detected logged-in user password change, reconnecting to server..."
argument_list|)
expr_stmt|;
name|that
operator|.
...
condition|)
block|{
comment|//register for password update event, if we are changing the password
