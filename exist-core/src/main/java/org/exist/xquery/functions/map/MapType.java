...
name|map
decl_stmt|;
comment|/**      * The type of the keys in the map,      * if not all keys have the same type      * then this is set to {@link #MIXED_KEY_TYPES}.      *      * Uses integer values from {@link org.exist.xquery.value.Type}.      */
specifier|private
name|int
name|keyType
init|=
...
return|;
block|}
comment|/**      * Construct a new Bifurcan mutable-map for use with AtomicValue keys.      *       * This function is predominantly for pre-building a Map of key/values      * for passing to {@link #MapType(XQueryContext, IMap, Integer)}.      *       * @param collator The collator if a collation is in effect for comparing keys.      *       * @return A mutable-map on which {@link IMap#forked()} can be called to produce an immutable map.       */
specifier|public
specifier|static
parameter_list|<
name|V
...
expr_stmt|;
break|break;
comment|// done, we only have to detect this once!
block|}
block|}
block|}
specifier|private
