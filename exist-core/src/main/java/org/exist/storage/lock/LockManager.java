...
specifier|static
name|String
name|CONFIGURATION_PATH_LOCKS_FOR_DOCUMENTS
init|=
literal|"lock-manager.document.use-path-locks"
decl_stmt|;
specifier|public
specifier|final
specifier|static
...
init|=
literal|"lock-manager.paths-multi-writer"
decl_stmt|;
...
init|=
literal|"exist.lockmanager.paths-multiwriter"
decl_stmt|;
specifier|public
specifier|final
specifier|static
...
argument_list|)
decl_stmt|;
comment|/**      * Set to true to use the path Hierarchy for document locks      * as opposed to separating Collection and Document locks      */
specifier|private
specifier|final
name|boolean
name|usePathLocksForDocuments
decl_stmt|;
comment|/**      * Set to true to enable Multi-Writer/Multi-Reader semantics for      * the path Hierarchy as opposed to the default Single-Writer/Multi-Reader      */
specifier|private
specifier|final
name|boolean
...
name|info
argument_list|(
literal|"Configured LockManager with concurrencyLevel={} use-path-locks-for-documents={} paths-multi-writer={}"
argument_list|,
name|concurrencyLevel
argument_list|,
name|usePathLocksForDocuments
...
return|;
block|}
comment|/**      * Creates a new lock for a Collection      * will be Striped by the collectionPath.      *      * @param collectionPath the collection path      *      * @return the document lock      */
specifier|private
specifier|static
name|MultiLock
name|createCollectionLock
...
return|;
block|}
comment|/**      * Creates a new MultiLock lock for a Document      * will be Striped by the documentPath.      *      * @param documentPath the document path      *      * @return the document lock      */
specifier|private
specifier|static
name|MultiLock
name|createDocumentLock
...
return|;
block|}
comment|/**      * Retrieves a lock for a Path      *      * This function is concerned with just the lock object      * and has no knowledge of the state of the lock. The only      * guarantee is that if this lock has not been requested before      * then it will be provided in the unlocked state      *      * @param path The path for which a lock is requested      *      * @return A lock for the path      */
name|MultiLock
name|getPathLock
parameter_list|(
specifier|final
...
block|}
block|}
comment|/**      * Acquires a READ_LOCK on a database path (and implicitly all descendant paths).      *      * @param lockType The type of the lock      * @param path The path for which a lock is requested.      *      * @return A READ_LOCK on the Collection.      *      * @throws LockException if a lock error occurs      */
specifier|public
name|LockGroup
name|acquirePathReadLock
parameter_list|(
...
return|;
block|}
comment|/**      * Acquires a WRITE_LOCK on a path (and implicitly all descendant paths).      *      * @param path The path for which a lock is requested.      * @param lockParent true if we should also explicitly write lock the parent path.      *      * @return A WRITE_LOCK on the path.      */
name|LockGroup
name|acquirePathWriteLock
parameter_list|(
specifier|final
...
block|}
block|}
comment|/**      * Gets a configuration option from an eXist-db Configuration file      * property.      *      * @param configuration eXist-db configuration      * @param configProperty name of an eXist-db configuration property      * @param defaultValue the default value if no system property of config property is found.      *      * @return the value of the property      */
specifier|static
name|boolean
name|getConfigPropertyBool
parameter_list|(
