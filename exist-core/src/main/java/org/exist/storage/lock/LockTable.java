...
init|=
literal|"lock-table.trace-stack-depth"
decl_stmt|;
...
return|;
block|}
comment|// try to upgrade optimistic-read or read lock to write lock
name|stamp
operator|=
name|entriesLock
operator|.
...
condition|)
block|{
comment|// release the read lock before taking the write lock
name|entriesLock
operator|.
name|unlockRead
argument_list|(
...
expr_stmt|;
block|}
comment|// take the write lock (blocking)
name|stamp
operator|=
name|entriesLock
operator|.
...
argument_list|()
expr_stmt|;
comment|// we must refresh the `local` as it could have changed between releasing the readLock and obtaining the write lock
name|local
operator|=
name|entries
operator|.
...
block|}
block|}
comment|// we have a write lock, add it
specifier|final
name|Entry
name|acquiredEntry
init|=
...
block|{
comment|// we don't need to unlock if it was just an optimistic read
if|if
condition|(
name|stamp
operator|!=
...
expr_stmt|;
block|}
comment|// unable to remove by tryConvertToWriteLock above, so directly acquire write lock
if|if
condition|(
name|mustRemove
condition|)
