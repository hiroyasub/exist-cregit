begin_unit|revision:1.0.0;language:Java;cregit-version:0.0.1
begin_package
package|package
name|org
operator|.
name|exist
operator|.
name|xquery
operator|.
name|functions
operator|.
name|util
package|;
end_package

begin_import
import|import
name|com
operator|.
name|evolvedbinary
operator|.
name|j8fu
operator|.
name|tuple
operator|.
name|Tuple2
import|;
end_import

begin_import
import|import
name|com
operator|.
name|googlecode
operator|.
name|junittoolbox
operator|.
name|ParallelRunner
import|;
end_import

begin_import
import|import
name|org
operator|.
name|custommonkey
operator|.
name|xmlunit
operator|.
name|exceptions
operator|.
name|XpathException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|exist
operator|.
name|test
operator|.
name|ExistXmldbEmbeddedServer
import|;
end_import

begin_import
import|import
name|org
operator|.
name|junit
operator|.
name|ClassRule
import|;
end_import

begin_import
import|import
name|org
operator|.
name|junit
operator|.
name|Test
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|custommonkey
operator|.
name|xmlunit
operator|.
name|XMLAssert
operator|.
name|assertXpathEvaluatesTo
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|junit
operator|.
name|Assert
operator|.
name|*
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|custommonkey
operator|.
name|xmlunit
operator|.
name|XMLAssert
operator|.
name|assertXMLEqual
import|;
end_import

begin_import
import|import
name|org
operator|.
name|junit
operator|.
name|runner
operator|.
name|RunWith
import|;
end_import

begin_import
import|import
name|org
operator|.
name|xml
operator|.
name|sax
operator|.
name|SAXException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|IOException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|List
import|;
end_import

begin_import
import|import
name|org
operator|.
name|custommonkey
operator|.
name|xmlunit
operator|.
name|XMLUnit
import|;
end_import

begin_import
import|import
name|org
operator|.
name|exist
operator|.
name|xquery
operator|.
name|XPathException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|xmldb
operator|.
name|api
operator|.
name|base
operator|.
name|ResourceSet
import|;
end_import

begin_import
import|import
name|org
operator|.
name|xmldb
operator|.
name|api
operator|.
name|base
operator|.
name|XMLDBException
import|;
end_import

begin_comment
comment|/**  *  * @author jim.fuller@webcomposite.com  */
end_comment

begin_class
annotation|@
name|RunWith
argument_list|(
name|ParallelRunner
operator|.
name|class
argument_list|)
specifier|public
class|class
name|SerializeTest
block|{
annotation|@
name|ClassRule
specifier|public
specifier|static
specifier|final
name|ExistXmldbEmbeddedServer
name|existEmbeddedServer
init|=
operator|new
name|ExistXmldbEmbeddedServer
argument_list|(
literal|false
argument_list|,
literal|true
argument_list|)
decl_stmt|;
annotation|@
name|Test
specifier|public
name|void
name|testSerialize
parameter_list|()
throws|throws
name|XPathException
throws|,
name|XMLDBException
throws|,
name|IOException
throws|,
name|SAXException
block|{
specifier|final
name|String
name|query
init|=
literal|"let $xml :=<test/>\n"
operator|+
literal|"return\n"
operator|+
literal|"util:serialize($xml,'method=xml')"
decl_stmt|;
specifier|final
name|ResourceSet
name|result
init|=
name|existEmbeddedServer
operator|.
name|executeQuery
argument_list|(
name|query
argument_list|)
decl_stmt|;
specifier|final
name|String
name|r
init|=
operator|(
name|String
operator|)
name|result
operator|.
name|getResource
argument_list|(
literal|0
argument_list|)
operator|.
name|getContent
argument_list|()
decl_stmt|;
name|assertXMLEqual
argument_list|(
literal|"<test/>"
argument_list|,
name|r
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|testSerialize2
parameter_list|()
throws|throws
name|XPathException
throws|,
name|XMLDBException
throws|,
name|IOException
throws|,
name|SAXException
block|{
specifier|final
name|String
name|query
init|=
literal|"let $xml :=<test><a/><b/></test>\n"
operator|+
literal|"return\n"
operator|+
literal|"util:serialize($xml,'method=xml indent=no')"
decl_stmt|;
specifier|final
name|ResourceSet
name|result
init|=
name|existEmbeddedServer
operator|.
name|executeQuery
argument_list|(
name|query
argument_list|)
decl_stmt|;
specifier|final
name|String
name|r
init|=
operator|(
name|String
operator|)
name|result
operator|.
name|getResource
argument_list|(
literal|0
argument_list|)
operator|.
name|getContent
argument_list|()
decl_stmt|;
name|assertXMLEqual
argument_list|(
literal|"<test><a/><b/></test>"
argument_list|,
name|r
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|testSerializeText
parameter_list|()
throws|throws
name|XPathException
throws|,
name|XMLDBException
throws|,
name|IOException
throws|,
name|SAXException
block|{
specifier|final
name|String
name|query
init|=
literal|"let $xml :=<test><a>test</a></test>\n"
operator|+
literal|"return\n"
operator|+
literal|"util:serialize($xml,'method=text indent=no')"
decl_stmt|;
specifier|final
name|ResourceSet
name|result
init|=
name|existEmbeddedServer
operator|.
name|executeQuery
argument_list|(
name|query
argument_list|)
decl_stmt|;
specifier|final
name|String
name|r
init|=
operator|(
name|String
operator|)
name|result
operator|.
name|getResource
argument_list|(
literal|0
argument_list|)
operator|.
name|getContent
argument_list|()
decl_stmt|;
name|assertEquals
argument_list|(
literal|"test"
argument_list|,
name|r
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|testSerializeIndent
parameter_list|()
throws|throws
name|XPathException
throws|,
name|XMLDBException
throws|,
name|IOException
throws|,
name|SAXException
block|{
specifier|final
name|String
name|query
init|=
literal|"let $xml :=<test><a/><b/></test>\n"
operator|+
literal|"return\n"
operator|+
literal|"util:serialize($xml,'method=xml indent=yes')"
decl_stmt|;
specifier|final
name|ResourceSet
name|result
init|=
name|existEmbeddedServer
operator|.
name|executeQuery
argument_list|(
name|query
argument_list|)
decl_stmt|;
specifier|final
name|String
name|r
init|=
operator|(
name|String
operator|)
name|result
operator|.
name|getResource
argument_list|(
literal|0
argument_list|)
operator|.
name|getContent
argument_list|()
decl_stmt|;
name|XMLUnit
operator|.
name|setIgnoreWhitespace
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|assertXMLEqual
argument_list|(
literal|"<test><a/><b/></test>"
argument_list|,
name|r
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|testSerializeXincludes
parameter_list|()
throws|throws
name|XPathException
throws|,
name|XMLDBException
throws|,
name|IOException
throws|,
name|SAXException
throws|,
name|XpathException
block|{
specifier|final
name|String
name|query
init|=
literal|"let $xml := "
operator|+
literal|"<test xmlns:xi='http://www.w3.org/2001/XInclude'>"
operator|+
literal|"<xi:include href='/db/system/security/config.xml'/>"
operator|+
literal|"</test>\n"
operator|+
literal|"return\n"
operator|+
literal|"util:serialize($xml,'enable-xincludes=yes')"
decl_stmt|;
specifier|final
name|ResourceSet
name|result
init|=
name|existEmbeddedServer
operator|.
name|executeQuery
argument_list|(
name|query
argument_list|)
decl_stmt|;
specifier|final
name|String
name|r
init|=
operator|(
name|String
operator|)
name|result
operator|.
name|getResource
argument_list|(
literal|0
argument_list|)
operator|.
name|getContent
argument_list|()
decl_stmt|;
name|assertXpathEvaluatesTo
argument_list|(
literal|"2.1"
argument_list|,
literal|"/test//@version"
argument_list|,
name|r
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|parseOption
parameter_list|()
block|{
specifier|final
name|List
argument_list|<
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
argument_list|>
name|result
init|=
name|Serialize
operator|.
name|parseOption
argument_list|(
literal|"method=text"
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|1
argument_list|,
name|result
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv
init|=
name|result
operator|.
name|get
argument_list|(
literal|0
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"method"
argument_list|,
name|kv
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"text"
argument_list|,
name|kv
operator|.
name|_2
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|parseOption_docTypePublic
parameter_list|()
block|{
specifier|final
name|List
argument_list|<
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
argument_list|>
name|result
init|=
name|Serialize
operator|.
name|parseOption
argument_list|(
literal|"doctype-public=-//D//DTD P-Topic//EN"
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|1
argument_list|,
name|result
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv
init|=
name|result
operator|.
name|get
argument_list|(
literal|0
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"doctype-public"
argument_list|,
name|kv
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"-//D//DTD P-Topic//EN"
argument_list|,
name|kv
operator|.
name|_2
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|parseOption_docTypeSystem
parameter_list|()
block|{
specifier|final
name|List
argument_list|<
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
argument_list|>
name|result
init|=
name|Serialize
operator|.
name|parseOption
argument_list|(
literal|"doctype-system=http://domain.com/xmetal/schemas/p-topic.dtd"
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|1
argument_list|,
name|result
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv
init|=
name|result
operator|.
name|get
argument_list|(
literal|0
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"doctype-system"
argument_list|,
name|kv
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"http://domain.com/xmetal/schemas/p-topic.dtd"
argument_list|,
name|kv
operator|.
name|_2
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|parseOption_multiSingleString
parameter_list|()
block|{
specifier|final
name|List
argument_list|<
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
argument_list|>
name|result
init|=
name|Serialize
operator|.
name|parseOption
argument_list|(
literal|"method=text indent=no"
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|2
argument_list|,
name|result
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv1
init|=
name|result
operator|.
name|get
argument_list|(
literal|0
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"method"
argument_list|,
name|kv1
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"text"
argument_list|,
name|kv1
operator|.
name|_2
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv2
init|=
name|result
operator|.
name|get
argument_list|(
literal|1
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"indent"
argument_list|,
name|kv2
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"no"
argument_list|,
name|kv2
operator|.
name|_2
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|parseOption_multiSingleStringWithSpace
parameter_list|()
block|{
specifier|final
name|List
argument_list|<
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
argument_list|>
name|result
init|=
name|Serialize
operator|.
name|parseOption
argument_list|(
literal|"doctype-public=-//D//DTD P-Topic//EN method=text indent=no"
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|3
argument_list|,
name|result
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv
init|=
name|result
operator|.
name|get
argument_list|(
literal|0
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"doctype-public"
argument_list|,
name|kv
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"-//D//DTD P-Topic//EN"
argument_list|,
name|kv
operator|.
name|_2
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv2
init|=
name|result
operator|.
name|get
argument_list|(
literal|1
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"method"
argument_list|,
name|kv2
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"text"
argument_list|,
name|kv2
operator|.
name|_2
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv3
init|=
name|result
operator|.
name|get
argument_list|(
literal|2
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"indent"
argument_list|,
name|kv3
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"no"
argument_list|,
name|kv3
operator|.
name|_2
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|parseOption_multiSingleStringWithSpace2
parameter_list|()
block|{
specifier|final
name|List
argument_list|<
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
argument_list|>
name|result
init|=
name|Serialize
operator|.
name|parseOption
argument_list|(
literal|"method=text doctype-public=-//D//DTD P-Topic//EN indent=no"
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|3
argument_list|,
name|result
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv1
init|=
name|result
operator|.
name|get
argument_list|(
literal|0
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"method"
argument_list|,
name|kv1
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"text"
argument_list|,
name|kv1
operator|.
name|_2
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv2
init|=
name|result
operator|.
name|get
argument_list|(
literal|1
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"doctype-public"
argument_list|,
name|kv2
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"-//D//DTD P-Topic//EN"
argument_list|,
name|kv2
operator|.
name|_2
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv3
init|=
name|result
operator|.
name|get
argument_list|(
literal|2
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"indent"
argument_list|,
name|kv3
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"no"
argument_list|,
name|kv3
operator|.
name|_2
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
specifier|public
name|void
name|parseOption_multiSingleStringWithSpace3
parameter_list|()
block|{
specifier|final
name|List
argument_list|<
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
argument_list|>
name|result
init|=
name|Serialize
operator|.
name|parseOption
argument_list|(
literal|"method=text indent=no doctype-public=-//D//DTD P-Topic//EN"
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|3
argument_list|,
name|result
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv1
init|=
name|result
operator|.
name|get
argument_list|(
literal|0
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"method"
argument_list|,
name|kv1
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"text"
argument_list|,
name|kv1
operator|.
name|_2
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv2
init|=
name|result
operator|.
name|get
argument_list|(
literal|1
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"indent"
argument_list|,
name|kv2
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"no"
argument_list|,
name|kv2
operator|.
name|_2
argument_list|)
expr_stmt|;
specifier|final
name|Tuple2
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|kv3
init|=
name|result
operator|.
name|get
argument_list|(
literal|2
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
literal|"doctype-public"
argument_list|,
name|kv3
operator|.
name|_1
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"-//D//DTD P-Topic//EN"
argument_list|,
name|kv3
operator|.
name|_2
argument_list|)
expr_stmt|;
block|}
block|}
end_class

end_unit

