...
return|;
block|}
comment|/** 	 * The hash code computation can be expensive, and the hash codes may not be very well distributed. 	 * You probably shouldn't use items in situations where they might get hashed. 	 */
annotation|@
name|Override
specifier|public
name|int
...
return|;
block|}
comment|/** 	 * Atomize this item and return the result.  This is useful if you don't know if you're 	 * holding a node or an atomic item, and want to ensure it's an atomic value without 	 * potentially losing its type by converting it to a string. 	 *  	 * @return the atomized value of this item; may be this item itself if it's already atomic 	 */
specifier|public
name|Item
name|toAtomicItem
parameter_list|()
...
name|DatabaseException
argument_list|(
literal|"unable to atomize item"
argument_list|,
name|e
argument_list|)
throw|;
...
return|;
block|}
comment|/** 	 * Return the comparable value of this item, if available.  The resulting object will 	 * directly compare the XQuery value without converting to a string first, unless the 	 * item is untyped and atomic, in which case it will be cast to a string.  Items of 	 * different types will compare in an arbitrary but stable order.  Nodes are never 	 * comparable. 	 * 	 * @return the comparable value of this item 	 * @throws DatabaseException if this item is not comparable 	 */
annotation|@
...
name|DatabaseException
argument_list|(
literal|"unable to convert to comparable value"
argument_list|,
name|e
argument_list|)
throw|;
