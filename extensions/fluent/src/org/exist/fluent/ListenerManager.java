...
try|try
block|{
comment|// We try to merge in our trigger if a config already exists, but if something goes wrong we'll just blow it away instead.
name|Document
name|configDoc
init|=
name|rootConfigFolder
...
condition|)
block|{
comment|// If no<collection> at root, let the exception take us to the "overwrite" path since the config isn't valid anyway.
name|Node
name|configRoot
init|=
name|configDoc
...
argument_list|()
expr_stmt|;
comment|// Hack: collection config will only refresh on new document store, so move the document away and back.
name|configDoc
operator|.
name|move
argument_list|(
...
name|warn
argument_list|(
literal|"forced to overwrite root collection config due to exception"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
literal|"event"
argument_list|,
literal|"store update remove create-collection rename-collection delete-collection"
argument_list|)
operator|.
name|attr
argument_list|(
