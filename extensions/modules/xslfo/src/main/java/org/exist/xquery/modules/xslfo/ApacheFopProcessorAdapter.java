begin_unit|revision:1.0.0;language:Java;cregit-version:0.0.1
begin_package
package|package
name|org
operator|.
name|exist
operator|.
name|xquery
operator|.
name|modules
operator|.
name|xslfo
package|;
end_package

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|OutputStream
import|;
end_import

begin_import
import|import
name|java
operator|.
name|net
operator|.
name|URI
import|;
end_import

begin_import
import|import
name|java
operator|.
name|net
operator|.
name|URISyntaxException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|HashSet
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Map
operator|.
name|Entry
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Objects
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Properties
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Set
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|avalon
operator|.
name|framework
operator|.
name|configuration
operator|.
name|Configuration
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|avalon
operator|.
name|framework
operator|.
name|configuration
operator|.
name|SAXConfigurationHandler
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|apps
operator|.
name|*
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|apps
operator|.
name|io
operator|.
name|ResourceResolverFactory
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|configuration
operator|.
name|ConfigurationException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|events
operator|.
name|Event
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|events
operator|.
name|EventFormatter
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|events
operator|.
name|EventListener
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|events
operator|.
name|model
operator|.
name|EventSeverity
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|logging
operator|.
name|log4j
operator|.
name|Logger
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|logging
operator|.
name|log4j
operator|.
name|LogManager
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|xmlgraphics
operator|.
name|io
operator|.
name|ResourceResolver
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|xmlgraphics
operator|.
name|io
operator|.
name|URIResolverAdapter
import|;
end_import

begin_import
import|import
name|org
operator|.
name|exist
operator|.
name|storage
operator|.
name|DBBroker
import|;
end_import

begin_import
import|import
name|org
operator|.
name|exist
operator|.
name|xslt
operator|.
name|EXistURIResolver
import|;
end_import

begin_import
import|import
name|org
operator|.
name|exist
operator|.
name|xquery
operator|.
name|value
operator|.
name|NodeValue
import|;
end_import

begin_import
import|import
name|org
operator|.
name|xml
operator|.
name|sax
operator|.
name|ContentHandler
import|;
end_import

begin_import
import|import
name|org
operator|.
name|xml
operator|.
name|sax
operator|.
name|SAXException
import|;
end_import

begin_import
import|import
name|javax
operator|.
name|annotation
operator|.
name|Nullable
import|;
end_import

begin_import
import|import
name|javax
operator|.
name|xml
operator|.
name|transform
operator|.
name|Source
import|;
end_import

begin_import
import|import
name|javax
operator|.
name|xml
operator|.
name|transform
operator|.
name|TransformerException
import|;
end_import

begin_import
import|import
name|javax
operator|.
name|xml
operator|.
name|transform
operator|.
name|URIResolver
import|;
end_import

begin_comment
comment|/**  * @author<a href="mailto:adam@exist-db.org">Adam Retter</a>  */
end_comment

begin_class
specifier|public
class|class
name|ApacheFopProcessorAdapter
implements|implements
name|ProcessorAdapter
block|{
specifier|private
specifier|static
specifier|final
name|Logger
name|LOG
init|=
name|LogManager
operator|.
name|getLogger
argument_list|(
name|ApacheFopProcessorAdapter
operator|.
name|class
argument_list|)
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|DEFAULT_BASE_URI
init|=
literal|"exist://localhost/db/"
decl_stmt|;
annotation|@
name|Override
specifier|public
name|ContentHandler
name|getContentHandler
parameter_list|(
specifier|final
name|DBBroker
name|broker
parameter_list|,
specifier|final
name|NodeValue
name|configFile
parameter_list|,
specifier|final
name|Properties
name|parameters
parameter_list|,
specifier|final
name|String
name|mimeType
parameter_list|,
specifier|final
name|OutputStream
name|os
parameter_list|)
throws|throws
name|SAXException
block|{
comment|// setup the FopFactory
specifier|final
name|FopFactoryBuilder
name|builder
decl_stmt|;
try|try
block|{
if|if
condition|(
name|configFile
operator|!=
literal|null
condition|)
block|{
specifier|final
name|FopConfigurationBuilder
name|cfgBuilder
init|=
operator|new
name|FopConfigurationBuilder
argument_list|(
name|broker
argument_list|)
decl_stmt|;
specifier|final
name|Configuration
name|cfg
init|=
name|cfgBuilder
operator|.
name|buildFromNode
argument_list|(
name|configFile
argument_list|)
decl_stmt|;
specifier|final
name|URI
name|defaultBaseURI
decl_stmt|;
if|if
condition|(
name|configFile
operator|instanceof
name|org
operator|.
name|exist
operator|.
name|dom
operator|.
name|memtree
operator|.
name|NodeImpl
condition|)
block|{
comment|//in-memory documents don't have a BaseURI
name|defaultBaseURI
operator|=
operator|new
name|URI
argument_list|(
name|DEFAULT_BASE_URI
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|defaultBaseURI
operator|=
operator|new
name|URI
argument_list|(
literal|"exist://localhost"
operator|+
name|configFile
operator|.
name|getOwnerDocument
argument_list|()
operator|.
name|getBaseURI
argument_list|()
argument_list|)
expr_stmt|;
block|}
specifier|final
name|EnvironmentProfile
name|environment
init|=
name|EnvironmentalProfileFactory
operator|.
name|createDefault
argument_list|(
name|defaultBaseURI
argument_list|,
name|getResourceResolver
argument_list|(
name|broker
argument_list|,
name|defaultBaseURI
operator|.
name|toString
argument_list|()
argument_list|)
argument_list|)
decl_stmt|;
name|builder
operator|=
operator|new
name|FopFactoryBuilder
argument_list|(
name|environment
argument_list|)
operator|.
name|setConfiguration
argument_list|(
operator|new
name|FopAvalonConfigurationAdapter
argument_list|(
name|cfg
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
specifier|final
name|URI
name|defaultBaseURI
init|=
operator|new
name|URI
argument_list|(
name|DEFAULT_BASE_URI
argument_list|)
decl_stmt|;
specifier|final
name|EnvironmentProfile
name|environment
init|=
name|EnvironmentalProfileFactory
operator|.
name|createDefault
argument_list|(
name|defaultBaseURI
argument_list|,
name|getResourceResolver
argument_list|(
name|broker
argument_list|,
name|defaultBaseURI
operator|.
name|toString
argument_list|()
argument_list|)
argument_list|)
decl_stmt|;
name|builder
operator|=
operator|new
name|FopFactoryBuilder
argument_list|(
name|environment
argument_list|)
expr_stmt|;
block|}
specifier|final
name|FopFactory
name|fopFactory
init|=
name|builder
operator|.
name|build
argument_list|()
decl_stmt|;
comment|// setup the foUserAgent, using given parameters held in the
comment|// transformer handler
specifier|final
name|FOUserAgent
name|foUserAgent
init|=
name|setupFOUserAgent
argument_list|(
name|fopFactory
operator|.
name|newFOUserAgent
argument_list|()
argument_list|,
name|parameters
argument_list|)
decl_stmt|;
name|foUserAgent
operator|.
name|getEventBroadcaster
argument_list|()
operator|.
name|addEventListener
argument_list|(
operator|new
name|ExistLoggingEventListener
argument_list|(
name|LOG
argument_list|)
argument_list|)
expr_stmt|;
comment|// create new instance of FOP using the mimetype, the created user
comment|// agent, and the output stream
specifier|final
name|Fop
name|fop
init|=
name|fopFactory
operator|.
name|newFop
argument_list|(
name|mimeType
argument_list|,
name|foUserAgent
argument_list|,
name|os
argument_list|)
decl_stmt|;
comment|// Obtain FOP's DefaultHandler
return|return
name|fop
operator|.
name|getDefaultHandler
argument_list|()
return|;
block|}
catch|catch
parameter_list|(
specifier|final
name|URISyntaxException
name|e
parameter_list|)
block|{
throw|throw
operator|new
name|SAXException
argument_list|(
literal|"Unable to parse baseURI"
argument_list|,
name|e
argument_list|)
throw|;
block|}
block|}
annotation|@
name|Override
specifier|public
name|void
name|cleanup
parameter_list|()
block|{
block|}
comment|/**      * Setup the UserAgent for FOP, from given parameters *      *      * @param foUserAgent The user agent to set parameters for      * @param parameters      *            any user defined parameters to the XSL-FO process      * @return FOUserAgent The generated FOUserAgent to include any parameters      *         passed in      */
specifier|private
name|FOUserAgent
name|setupFOUserAgent
parameter_list|(
specifier|final
name|FOUserAgent
name|foUserAgent
parameter_list|,
specifier|final
name|Properties
name|parameters
parameter_list|)
block|{
comment|// setup the foUserAgent as per the parameters given
name|foUserAgent
operator|.
name|setProducer
argument_list|(
literal|"eXist-db with Apache FOP"
argument_list|)
expr_stmt|;
if|if
condition|(
name|parameters
operator|!=
literal|null
condition|)
block|{
for|for
control|(
specifier|final
name|Entry
name|paramEntry
range|:
name|parameters
operator|.
name|entrySet
argument_list|()
control|)
block|{
specifier|final
name|String
name|key
init|=
operator|(
name|String
operator|)
name|paramEntry
operator|.
name|getKey
argument_list|()
decl_stmt|;
specifier|final
name|String
name|value
init|=
operator|(
name|String
operator|)
name|paramEntry
operator|.
name|getValue
argument_list|()
decl_stmt|;
if|if
condition|(
name|key
operator|.
name|equals
argument_list|(
literal|"FOPauthor"
argument_list|)
condition|)
block|{
name|foUserAgent
operator|.
name|setAuthor
argument_list|(
name|value
argument_list|)
expr_stmt|;
block|}
if|else if
condition|(
name|key
operator|.
name|equals
argument_list|(
literal|"FOPtitle"
argument_list|)
condition|)
block|{
name|foUserAgent
operator|.
name|setTitle
argument_list|(
name|value
argument_list|)
expr_stmt|;
block|}
if|else if
condition|(
name|key
operator|.
name|equals
argument_list|(
literal|"FOPkeywords"
argument_list|)
condition|)
block|{
name|foUserAgent
operator|.
name|setTitle
argument_list|(
name|value
argument_list|)
expr_stmt|;
block|}
if|else if
condition|(
name|key
operator|.
name|equals
argument_list|(
literal|"FOPdpi"
argument_list|)
condition|)
block|{
try|try
block|{
name|foUserAgent
operator|.
name|setTargetResolution
argument_list|(
name|Integer
operator|.
name|parseInt
argument_list|(
name|value
argument_list|)
argument_list|)
expr_stmt|;
block|}
catch|catch
parameter_list|(
specifier|final
name|NumberFormatException
name|nfe
parameter_list|)
block|{
name|LOG
operator|.
name|warn
argument_list|(
literal|"Unable to set DPI to: "
operator|+
name|value
argument_list|)
expr_stmt|;
block|}
block|}
block|}
block|}
return|return
name|foUserAgent
return|;
block|}
comment|/**      * Returns a scheme aware ResourceResolver which supports:      *   file://      *   exist:// (which will be translated to xmldb:exist://)      *   http://      *   https://      *      * @return The resource resolver      */
specifier|private
name|ResourceResolver
name|getResourceResolver
parameter_list|(
specifier|final
name|DBBroker
name|broker
parameter_list|,
specifier|final
name|String
name|baseUri
parameter_list|)
block|{
specifier|final
name|ResourceResolverFactory
operator|.
name|SchemeAwareResourceResolverBuilder
name|builder
init|=
name|ResourceResolverFactory
operator|.
name|createSchemeAwareResourceResolverBuilder
argument_list|(
name|ResourceResolverFactory
operator|.
name|createDefaultResourceResolver
argument_list|()
argument_list|)
decl_stmt|;
specifier|final
name|URIResolverAdapter
name|uriResolver
init|=
operator|new
name|URIResolverAdapter
argument_list|(
operator|new
name|ExistSchemeRewriter
argument_list|(
operator|new
name|EXistURIResolver
argument_list|(
name|broker
operator|.
name|getBrokerPool
argument_list|()
argument_list|,
name|baseUri
argument_list|)
argument_list|)
argument_list|)
decl_stmt|;
name|builder
operator|.
name|registerResourceResolverForScheme
argument_list|(
literal|"exist"
argument_list|,
name|uriResolver
argument_list|)
expr_stmt|;
name|builder
operator|.
name|registerResourceResolverForScheme
argument_list|(
literal|"http"
argument_list|,
name|uriResolver
argument_list|)
expr_stmt|;
name|builder
operator|.
name|registerResourceResolverForScheme
argument_list|(
literal|"https"
argument_list|,
name|uriResolver
argument_list|)
expr_stmt|;
return|return
name|builder
operator|.
name|build
argument_list|()
return|;
block|}
comment|/**      * Rewrites URLs like:      *  exist://localhost/db -> /db      */
specifier|private
specifier|static
class|class
name|ExistSchemeRewriter
implements|implements
name|URIResolver
block|{
specifier|private
specifier|final
name|EXistURIResolver
name|eXistURIResolver
decl_stmt|;
specifier|public
name|ExistSchemeRewriter
parameter_list|(
specifier|final
name|EXistURIResolver
name|eXistURIResolver
parameter_list|)
block|{
name|this
operator|.
name|eXistURIResolver
operator|=
name|eXistURIResolver
expr_stmt|;
block|}
annotation|@
name|Override
specifier|public
name|Source
name|resolve
parameter_list|(
specifier|final
name|String
name|href
parameter_list|,
specifier|final
name|String
name|base
parameter_list|)
throws|throws
name|TransformerException
block|{
return|return
name|eXistURIResolver
operator|.
name|resolve
argument_list|(
name|rewriteScheme
argument_list|(
name|href
argument_list|)
argument_list|,
name|rewriteScheme
argument_list|(
name|base
argument_list|)
argument_list|)
return|;
block|}
specifier|private
name|String
name|rewriteScheme
parameter_list|(
name|String
name|uri
parameter_list|)
block|{
if|if
condition|(
name|uri
operator|!=
literal|null
condition|)
block|{
if|if
condition|(
name|uri
operator|.
name|startsWith
argument_list|(
literal|"exist://localhost"
argument_list|)
condition|)
block|{
name|uri
operator|=
name|uri
operator|.
name|replace
argument_list|(
literal|"exist://localhost/db"
argument_list|,
literal|"/db"
argument_list|)
expr_stmt|;
block|}
if|else if
condition|(
name|uri
operator|.
name|startsWith
argument_list|(
literal|"exist://"
argument_list|)
condition|)
block|{
name|uri
operator|=
name|uri
operator|.
name|replace
argument_list|(
literal|"exist://"
argument_list|,
literal|"xmldb:exist://"
argument_list|)
expr_stmt|;
block|}
block|}
return|return
name|uri
return|;
block|}
block|}
comment|/**      * Extension of the Apache Avalon DefaultConfigurationBuilder Allows better      * integration with Nodes passed in from eXist as Configuration files      */
specifier|private
specifier|static
class|class
name|FopConfigurationBuilder
extends|extends
name|org
operator|.
name|apache
operator|.
name|avalon
operator|.
name|framework
operator|.
name|configuration
operator|.
name|DefaultConfigurationBuilder
block|{
specifier|private
specifier|final
name|DBBroker
name|broker
decl_stmt|;
specifier|public
name|FopConfigurationBuilder
parameter_list|(
specifier|final
name|DBBroker
name|broker
parameter_list|)
block|{
name|super
argument_list|()
expr_stmt|;
name|this
operator|.
name|broker
operator|=
name|broker
expr_stmt|;
block|}
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unused"
argument_list|)
specifier|public
name|FopConfigurationBuilder
parameter_list|(
specifier|final
name|DBBroker
name|broker
parameter_list|,
specifier|final
name|boolean
name|enableNamespaces
parameter_list|)
block|{
name|super
argument_list|(
name|enableNamespaces
argument_list|)
expr_stmt|;
name|this
operator|.
name|broker
operator|=
name|broker
expr_stmt|;
block|}
specifier|public
name|Configuration
name|buildFromNode
parameter_list|(
specifier|final
name|NodeValue
name|configFile
parameter_list|)
throws|throws
name|SAXException
block|{
specifier|final
name|SAXConfigurationHandler
name|handler
init|=
name|getHandler
argument_list|()
decl_stmt|;
name|handler
operator|.
name|clear
argument_list|()
expr_stmt|;
name|configFile
operator|.
name|toSAX
argument_list|(
name|broker
argument_list|,
name|handler
argument_list|,
operator|new
name|Properties
argument_list|()
argument_list|)
expr_stmt|;
return|return
name|handler
operator|.
name|getConfiguration
argument_list|()
return|;
block|}
block|}
comment|/**      * Adapter between Avalon config which lets us easily use a Node as the configuration      * source, and Apache Fop configuration which does not.      *      * This was needed after the Fop API changed from version 2.3 to 2.4.      */
specifier|private
specifier|static
class|class
name|FopAvalonConfigurationAdapter
implements|implements
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|configuration
operator|.
name|Configuration
block|{
specifier|private
specifier|final
name|Configuration
name|avalonConfiguration
decl_stmt|;
specifier|public
name|FopAvalonConfigurationAdapter
parameter_list|(
specifier|final
name|Configuration
name|avalonConfiguration
parameter_list|)
block|{
name|this
operator|.
name|avalonConfiguration
operator|=
name|Objects
operator|.
name|requireNonNull
argument_list|(
name|avalonConfiguration
argument_list|,
literal|"Avalon Configuration must not be null"
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Override
specifier|public
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|configuration
operator|.
name|Configuration
name|getChild
parameter_list|(
specifier|final
name|String
name|child
parameter_list|)
block|{
return|return
operator|new
name|FopAvalonConfigurationAdapter
argument_list|(
name|avalonConfiguration
operator|.
name|getChild
argument_list|(
name|child
argument_list|)
argument_list|)
return|;
block|}
annotation|@
name|Override
specifier|public
annotation|@
name|Nullable
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|configuration
operator|.
name|Configuration
name|getChild
parameter_list|(
specifier|final
name|String
name|child
parameter_list|,
name|boolean
name|createNew
parameter_list|)
block|{
specifier|final
name|Configuration
name|childConfig
init|=
name|avalonConfiguration
operator|.
name|getChild
argument_list|(
name|child
argument_list|,
name|createNew
argument_list|)
decl_stmt|;
if|if
condition|(
name|childConfig
operator|==
literal|null
condition|)
block|{
return|return
literal|null
return|;
block|}
return|return
operator|new
name|FopAvalonConfigurationAdapter
argument_list|(
name|childConfig
argument_list|)
return|;
block|}
annotation|@
name|Override
specifier|public
annotation|@
name|Nullable
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|configuration
operator|.
name|Configuration
index|[]
name|getChildren
parameter_list|(
specifier|final
name|String
name|name
parameter_list|)
block|{
specifier|final
name|Configuration
index|[]
name|children
init|=
name|avalonConfiguration
operator|.
name|getChildren
argument_list|(
name|name
argument_list|)
decl_stmt|;
if|if
condition|(
name|children
operator|==
literal|null
condition|)
block|{
return|return
literal|null
return|;
block|}
specifier|final
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|configuration
operator|.
name|Configuration
index|[]
name|newChildren
init|=
operator|new
name|org
operator|.
name|apache
operator|.
name|fop
operator|.
name|configuration
operator|.
name|Configuration
index|[
name|children
operator|.
name|length
index|]
decl_stmt|;
for|for
control|(
name|int
name|i
init|=
literal|0
init|;
name|i
operator|<
name|children
operator|.
name|length
condition|;
name|i
operator|++
control|)
block|{
name|newChildren
index|[
name|i
index|]
operator|=
operator|new
name|FopAvalonConfigurationAdapter
argument_list|(
name|children
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
return|return
name|newChildren
return|;
block|}
annotation|@
name|Override
specifier|public
name|String
index|[]
name|getAttributeNames
parameter_list|()
block|{
return|return
name|avalonConfiguration
operator|.
name|getAttributeNames
argument_list|()
return|;
block|}
annotation|@
name|Override
specifier|public
name|String
name|getAttribute
parameter_list|(
specifier|final
name|String
name|paramName
parameter_list|)
throws|throws
name|ConfigurationException
block|{
try|try
block|{
return|return
name|avalonConfiguration
operator|.
name|getAttribute
argument_list|(
name|paramName
argument_list|)
return|;
block|}
catch|catch
parameter_list|(
specifier|final
name|org
operator|.
name|apache
operator|.
name|avalon
operator|.
name|framework
operator|.
name|configuration
operator|.
name|ConfigurationException
name|e
parameter_list|)
block|{
throw|throw
operator|new
name|ConfigurationException
argument_list|(
name|e
operator|.
name|getMessage
argument_list|()
argument_list|,
name|e
argument_list|)
throw|;
block|}
block|}
annotation|@
name|Override
specifier|public
name|String
name|getAttribute
parameter_list|(
specifier|final
name|String
name|name
parameter_list|,
specifier|final
name|String
name|defaultValue
parameter_list|)
block|{
return|return
name|avalonConfiguration
operator|.
name|getAttribute
argument_list|(
name|name
argument_list|,
name|defaultValue
argument_list|)
return|;
block|}
annotation|@
name|Override
specifier|public
name|boolean
name|getAttributeAsBoolean
parameter_list|(
specifier|final
name|String
name|name
parameter_list|,
specifier|final
name|boolean
name|defaultValue
parameter_list|)
block|{
return|return
name|avalonConfiguration
operator|.
name|getAttributeAsBoolean
argument_list|(
name|name
argument_list|,
name|defaultValue
argument_list|)
return|;
block|}
annotation|@
name|Override
specifier|public
name|float
name|getAttributeAsFloat
parameter_list|(
specifier|final
name|String
name|paramName
parameter_list|)
throws|throws
name|ConfigurationException
block|{
try|try
block|{
return|return
name|avalonConfiguration
operator|.
name|getAttributeAsFloat
argument_list|(
name|paramName
argument_list|)
return|;
block|}
catch|catch
parameter_list|(
specifier|final
name|org
operator|.
name|apache
operator|.
name|avalon
operator|.
name|framework
operator|.
name|configuration
operator|.
name|ConfigurationException
name|e
parameter_list|)
block|{
throw|throw
operator|new
name|ConfigurationException
argument_list|(
name|e
operator|.
name|getMessage
argument_list|()
argument_list|,
name|e
argument_list|)
throw|;
block|}
block|}
annotation|@
name|Override
specifier|public
name|float
name|getAttributeAsFloat
parameter_list|(
specifier|final
name|String
name|name
parameter_list|,
specifier|final
name|float
name|defaultValue
parameter_list|)
block|{
return|return
name|avalonConfiguration
operator|.
name|getAttributeAsFloat
argument_list|(
name|name
argument_list|,
name|defaultValue
argument_list|)
return|;
block|}
annotation|@
name|Override
specifier|public
name|int
name|getAttributeAsInteger
parameter_list|(
specifier|final
name|String
name|name
parameter_list|,
specifier|final
name|int
name|defaultValue
parameter_list|)
block|{
return|return
name|avalonConfiguration
operator|.
name|getAttributeAsInteger
argument_list|(
name|name
argument_list|,
name|defaultValue
argument_list|)
return|;
block|}
annotation|@
name|Override
specifier|public
name|String
name|getValue
parameter_list|()
throws|throws
name|ConfigurationException
block|{
try|try
block|{
return|return
name|avalonConfiguration
operator|.
name|getValue
argument_list|()
return|;
block|}
catch|catch
parameter_list|(
specifier|final
name|org
operator|.
name|apache
operator|.
name|avalon
operator|.
name|framework
operator|.
name|configuration
operator|.
name|ConfigurationException
name|e
parameter_list|)
block|{
throw|throw
operator|new
name|ConfigurationException
argument_list|(
name|e
operator|.
name|getMessage
argument_list|()
argument_list|,
name|e
argument_list|)
throw|;
block|}
block|}
annotation|@
name|Override
specifier|public
name|String
name|getValue
parameter_list|(
specifier|final
name|String
name|defaultValue
parameter_list|)
block|{
return|return
name|avalonConfiguration
operator|.
name|getValue
argument_list|(
name|defaultValue
argument_list|)
return|;
block|}
annotation|@
name|Override
specifier|public
name|boolean
name|getValueAsBoolean
parameter_list|()
throws|throws
name|ConfigurationException
block|{
try|try
block|{
return|return
name|avalonConfiguration
operator|.
name|getValueAsBoolean
argument_list|()
return|;
block|}
catch|catch
parameter_list|(
specifier|final
name|org
operator|.
name|apache
operator|.
name|avalon
operator|.
name|framework
operator|.
name|configuration
operator|.
name|ConfigurationException
name|e
parameter_list|)
block|{
throw|throw
operator|new
name|ConfigurationException
argument_list|(
name|e
operator|.
name|getMessage
argument_list|()
argument_list|,
name|e
argument_list|)
throw|;
block|}
block|}
annotation|@
name|Override
specifier|public
name|boolean
name|getValueAsBoolean
parameter_list|(
specifier|final
name|boolean
name|defaultValue
parameter_list|)
block|{
return|return
name|avalonConfiguration
operator|.
name|getValueAsBoolean
argument_list|(
name|defaultValue
argument_list|)
return|;
block|}
annotation|@
name|Override
specifier|public
name|int
name|getValueAsInteger
parameter_list|()
throws|throws
name|ConfigurationException
block|{
try|try
block|{
return|return
name|avalonConfiguration
operator|.
name|getValueAsInteger
argument_list|()
return|;
block|}
catch|catch
parameter_list|(
specifier|final
name|org
operator|.
name|apache
operator|.
name|avalon
operator|.
name|framework
operator|.
name|configuration
operator|.
name|ConfigurationException
name|e
parameter_list|)
block|{
throw|throw
operator|new
name|ConfigurationException
argument_list|(
name|e
operator|.
name|getMessage
argument_list|()
argument_list|,
name|e
argument_list|)
throw|;
block|}
block|}
annotation|@
name|Override
specifier|public
name|int
name|getValueAsInteger
parameter_list|(
specifier|final
name|int
name|defaultValue
parameter_list|)
block|{
return|return
name|avalonConfiguration
operator|.
name|getValueAsInteger
argument_list|(
name|defaultValue
argument_list|)
return|;
block|}
annotation|@
name|Override
specifier|public
name|float
name|getValueAsFloat
parameter_list|()
throws|throws
name|ConfigurationException
block|{
try|try
block|{
return|return
name|avalonConfiguration
operator|.
name|getValueAsFloat
argument_list|()
return|;
block|}
catch|catch
parameter_list|(
specifier|final
name|org
operator|.
name|apache
operator|.
name|avalon
operator|.
name|framework
operator|.
name|configuration
operator|.
name|ConfigurationException
name|e
parameter_list|)
block|{
throw|throw
operator|new
name|ConfigurationException
argument_list|(
name|e
operator|.
name|getMessage
argument_list|()
argument_list|,
name|e
argument_list|)
throw|;
block|}
block|}
annotation|@
name|Override
specifier|public
name|float
name|getValueAsFloat
parameter_list|(
specifier|final
name|float
name|defaultValue
parameter_list|)
block|{
return|return
name|avalonConfiguration
operator|.
name|getValueAsFloat
argument_list|(
name|defaultValue
argument_list|)
return|;
block|}
annotation|@
name|Override
specifier|public
name|String
name|getLocation
parameter_list|()
block|{
return|return
name|avalonConfiguration
operator|.
name|getLocation
argument_list|()
return|;
block|}
block|}
specifier|private
specifier|static
class|class
name|ExistLoggingEventListener
implements|implements
name|EventListener
block|{
specifier|private
specifier|final
name|Logger
name|log
decl_stmt|;
specifier|private
specifier|final
name|Set
argument_list|<
name|String
argument_list|>
name|loggedMessages
init|=
operator|new
name|HashSet
argument_list|<>
argument_list|()
decl_stmt|;
specifier|public
name|ExistLoggingEventListener
parameter_list|(
specifier|final
name|Logger
name|log
parameter_list|)
block|{
name|this
operator|.
name|log
operator|=
name|log
expr_stmt|;
block|}
annotation|@
name|Override
specifier|public
name|void
name|processEvent
parameter_list|(
specifier|final
name|Event
name|event
parameter_list|)
block|{
name|String
name|msg
init|=
name|EventFormatter
operator|.
name|format
argument_list|(
name|event
argument_list|)
decl_stmt|;
name|EventSeverity
name|severity
init|=
name|event
operator|.
name|getSeverity
argument_list|()
decl_stmt|;
if|if
condition|(
name|severity
operator|==
name|EventSeverity
operator|.
name|INFO
condition|)
block|{
name|log
operator|.
name|info
argument_list|(
name|msg
argument_list|)
expr_stmt|;
block|}
if|else if
condition|(
name|severity
operator|==
name|EventSeverity
operator|.
name|WARN
condition|)
block|{
comment|// we want to prevent logging of duplicate messages in situations where they are likely
comment|// to occur; for instance, warning related to layout do not repeat (since line number
comment|// will be different) and as such we do not try to filter them here; on the other hand,
comment|// font related warnings are very likely to repeat and we try to filter them out here;
comment|// the same may happen with missing images (but not implemented yet).
name|String
name|eventGroupID
init|=
name|event
operator|.
name|getEventGroupID
argument_list|()
decl_stmt|;
if|if
condition|(
name|eventGroupID
operator|.
name|equals
argument_list|(
literal|"org.apache.fop.fonts.FontEventProducer"
argument_list|)
condition|)
block|{
if|if
condition|(
operator|!
name|loggedMessages
operator|.
name|contains
argument_list|(
name|msg
argument_list|)
condition|)
block|{
name|loggedMessages
operator|.
name|add
argument_list|(
name|msg
argument_list|)
expr_stmt|;
name|log
operator|.
name|warn
argument_list|(
name|msg
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
name|log
operator|.
name|warn
argument_list|(
name|msg
argument_list|)
expr_stmt|;
block|}
block|}
if|else if
condition|(
name|severity
operator|==
name|EventSeverity
operator|.
name|ERROR
condition|)
block|{
if|if
condition|(
name|event
operator|.
name|getParam
argument_list|(
literal|"e"
argument_list|)
operator|!=
literal|null
condition|)
block|{
name|log
operator|.
name|error
argument_list|(
name|msg
argument_list|,
operator|(
name|Throwable
operator|)
name|event
operator|.
name|getParam
argument_list|(
literal|"e"
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|log
operator|.
name|error
argument_list|(
name|msg
argument_list|)
expr_stmt|;
block|}
block|}
if|else if
condition|(
name|severity
operator|==
name|EventSeverity
operator|.
name|FATAL
condition|)
block|{
if|if
condition|(
name|event
operator|.
name|getParam
argument_list|(
literal|"e"
argument_list|)
operator|!=
literal|null
condition|)
block|{
name|log
operator|.
name|fatal
argument_list|(
name|msg
argument_list|,
operator|(
name|Throwable
operator|)
name|event
operator|.
name|getParam
argument_list|(
literal|"e"
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|log
operator|.
name|fatal
argument_list|(
name|msg
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
assert|assert
literal|false
assert|;
block|}
block|}
block|}
block|}
end_class

end_unit

