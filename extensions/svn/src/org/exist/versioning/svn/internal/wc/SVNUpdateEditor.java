...
block|}
block|}
name|scheduleExistingEntryForReAdd
argument_list|(
name|entry
argument_list|,
name|fullPath
...
condition|)
block|{
comment|//          The item does not exist locally (except perhaps as a skeleton
comment|//          directory tree) because it was already scheduled for delete.
comment|//          We must complete the deletion, leaving the tree conflict info
comment|//          as the only difference from a normal deletion.
comment|//
...
argument_list|,
literal|"Unexpected tree conflict reason"
argument_list|)
decl_stmt|;
...
name|wrap
argument_list|(
literal|"Error running log file for ''{0}''"
argument_list|,
name|parentArea
operator|.
name|getRoot
...
block|{
name|boolean
name|allModsAreDeletes
init|=
literal|false
decl_stmt|;
name|boolean
name|isSubtreeOfLocallyDeleted
init|=
name|inDeletedTree
argument_list|(
name|path
...
name|modified
operator|=
name|entryHasLocalModifications
argument_list|(
name|parentArea
argument_list|,
name|path
...
name|modified
operator|=
name|treeHasLocalModifications
argument_list|(
name|adminArea
argument_list|,
name|allEditsAreDeletes
...
name|WC_OBSTRUCTED_UPDATE
argument_list|,
literal|"UUID mismatch: existing directory ''{0}'' was checked out from a different repository"
argument_list|,
name|myCurrentDirectory
operator|.
name|getPath
...
name|WC_OBSTRUCTED_UPDATE
argument_list|,
literal|"URL ''{0}'' of existing directory ''{1}'' does not match expected URL ''{2}''"
argument_list|,
operator|new
name|Object
index|[]
...
argument_list|,
literal|"Checksum mismatch for ''{0}''; expected: ''{1}'', recorded: ''{2}''"
argument_list|,
operator|new
name|Object
index|[]
...
name|WC_CORRUPT_TEXT_BASE
argument_list|,
literal|"Checksum mismatch while updating ''{0}''; expected: ''{1}'', actual: ''{2}''"
argument_list|,
operator|new
name|Object
index|[]
...
condition|)
block|{
comment|// special case of missing not yet versioned directory scheduled for addition.
comment|// remove this entry, no chance to restore anything from the directory.
name|adminArea
operator|.
name|deleteEntry
argument_list|(
...
name|WC_OBSTRUCTED_UPDATE
argument_list|,
literal|"Failed to add file ''{0}'': an unversioned file of the same name already exists"
argument_list|,
name|path
argument_list|)
decl_stmt|;
...
name|WC_OBSTRUCTED_UPDATE
argument_list|,
literal|"UUID mismatch: existing file ''{0}'' was checked out from a different repository"
argument_list|,
name|fullPath
argument_list|)
decl_stmt|;
...
name|WC_OBSTRUCTED_UPDATE
argument_list|,
literal|"URL ''{0}'' of existing file ''{1}'' does not match expected URL ''{2}''"
argument_list|,
operator|new
name|Object
index|[]
...
expr_stmt|;
block|}
comment|//                if (fileEntry == null || !fileEntry.isScheduledForDeletion()) {
comment|//                }
block|}
else|else
block|{
...
name|treeConficted
decl_stmt|;
comment|//     Set if this file is locally deleted or is being added
comment|//     within a locally deleted tree.
specifier|private
name|boolean
name|isDeleted
decl_stmt|;
comment|//     The checksum for the file located at newBaseFile.
comment|//private String actualChecksum;
comment|//     If this file was added with history, this is the checksum of the
comment|//     text base (see copied_text_base). May be NULL if unknown.
comment|//private String copiedBaseChecksum;
specifier|public
name|SVNFileInfo
parameter_list|(
name|SVNDirectoryInfo
