...
argument_list|()
decl_stmt|;
comment|//            if (contextSet != null&& contextSet.getItemCount() == 1) {
comment|//                NodeProxy node = contextSet.get(0);
comment|//                BytesRef lowerBound = new BytesRef(LuceneUtil.createId(node.getDoc().getDocId(), node.getNodeId()));
comment|//                BytesRef upperBound = new BytesRef(LuceneUtil.createId(node.getDoc().getDocId(), node.getNodeId().nextSibling()));
comment|//                TermRangeQuery rangeQuery = new TermRangeQuery(FIELD_ID, lowerBound, upperBound, true, true);
comment|//                query.add(rangeQuery, BooleanClause.Occur.MUST);
comment|//            }
name|int
name|j
init|=
...
condition|)
block|{
comment|//Filter filter = new TermsFilter(FIELD_ID, terms);
comment|//Filter filter = new NodesFilter(contextSet);
name|resultSet
operator|=
name|doQuery
argument_list|(
