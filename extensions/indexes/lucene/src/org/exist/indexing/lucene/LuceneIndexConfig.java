...
name|EXistException
argument_list|(
literal|"Not all attributes for element "
operator|+
name|element
operator|+
literal|" stored yet, cannot iterate further."
argument_list|)
throw|;
block|}
name|AttrImpl
...
control|)
block|{
comment|// if matchAttr value is null we dont care about the value
if|if
condition|(
name|matchAttrData
operator|.
...
name|boost
expr_stmt|;
comment|// we matched the attribute already, but since we allow more boost on same attribute, both from "has-attribute" and "match-attribute" there is no break here
block|}
block|}
block|}
block|}
...
name|warn
argument_list|(
literal|"Exception while iterating attribute nodes: "
operator|+
name|e
operator|.
name|getMessage
