...
condition|)
block|{
comment|//TODO remove this check as the primary group should never be null
name|Group
name|primaryGroup
init|=
name|getGroup
...
block|}
block|}
comment|/*     private byte[] longToBytes(long l) throws IOException {         DataOutputStream dos = null;         try {             ByteArrayOutputStream baos = new ByteArrayOutputStream();               dos = new DataOutputStream(baos);             dos.writeLong(l);             dos.flush();             return baos.toByteArray();         } finally {             dos.close();         }     }*/
comment|/*     private long bytesToLong(byte b[]) throws IOException {        long val = 0;        int startOffset = (b.length>= 8 ? 7 : b.length - 1); //restrict size to long i.e. 8 bytes        for(int i = startOffset; i> -1; i--) {            val |= (((long)b[i])<< ((startOffset - i) * 8));        }        return val;     }*/
comment|/**      *      * byte[0] - revision level      * byte[1-7] - 48 bit authority      * byte[8]- count of sub authorities      * and then count x 32 bit sub authorities      *       * S-Revision-Authority-SubAuths....      */
comment|/*     private String decodeSID(byte[] sid) {                  try {             StringBuilder decodedSid = new StringBuilder("S-");              //revision level             decodedSid.append(Integer.toString(sid[0]));              decodedSid.append("-");              //authority             byte[] authority = Arrays.copyOfRange(sid, 1, 7);             decodedSid.append(Long.toHexString(bytesToLong(authority)));              //get the count of sub authorities             int countSubAuths = sid[7];              int offset = 8;              for(int i = 0; i< countSubAuths; i++) {                 byte subAuth[] = Arrays.copyOfRange(sid, offset, offset+4);                 decodedSid.append("-");                 decodedSid.append(Long.toHexString(bytesToLong(subAuth)));                 offset+= 4;             }                          return decodedSid.toString();                      } catch(IOException ioe) {             LOG.error(ioe.getMessage(), ioe);             return null;         }     }*/
specifier|private
name|String
name|decodeSID
parameter_list|(
...
name|strPrimaryGroupID
return|;
comment|/*                  byte[] sid = objectSID.getBytes();         final byte primaryGroupId[];         try {             primaryGroupId = longToBytes(Long.parseLong(strPrimaryGroupID));         } catch(IOException ioe) {             LOG.error(ioe.getMessage(), ioe);             return null;         }                  for(int i = 0; i< primaryGroupId.length; i++) {             sid[sid.length - primaryGroupId.length - i] = primaryGroupId[i];         }                  return sid;*/
block|}
specifier|private
name|String
name|getSIDAsByteString
parameter_list|(
name|byte
index|[]
name|sid
...
name|j
decl_stmt|;
comment|//Convert the SID into string using the byte format
for|for
control|(
name|int
name|i
...
condition|)
block|{
comment|//add a leading zero, add two leading \\ to make it easy
comment|//to paste into subsequent searches
name|byteSID
operator|=
name|byteSID
operator|+
