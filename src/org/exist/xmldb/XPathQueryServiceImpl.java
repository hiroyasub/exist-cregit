...
name|XMLDBException
function_decl|;
comment|/**      * Declare an external XPath variable and assign a value to it.      *<p>      * A variable can be referenced inside an XPath expression as      *<b>$variable</b>. For example, if you declare a variable with      *<p>      *<pre>      * 	declareVariable("name", "HAMLET");      *</pre>      *<p>      * you may use the variable in an XPath expression as follows:      *<p>      *<pre>      * 	//SPEECH[SPEAKER=$name]      *</pre>      *<p>      * Any Java object may be passed as initial value. The query engine will try      * to map this into a corresponding XPath value. You may also pass an      * XMLResource as obtained from another XPath expression. This will be      * converted into a node.      *      * @param qname        a valid QName by which the variable is identified. Any      *                     prefix should have been mapped to a namespace, i.e. if a variable is called      *<b>x:name</b>, there should be a prefix/namespace mapping for the prefix      *                     x      * @param initialValue the initial value, which is assigned to the variable      * @throws XMLDBException      */
name|void
...
name|XMLDBException
function_decl|;
comment|/**      * Close the protected environment. All locks held      * by the current thread are released. The query result set      * is no longer guaranteed to be stable.      *<p>      * Note: if beginProtected was used, you have to make sure      * endProtected is called in ALL cases. Otherwise some resource      * locks may not be released properly.      */
name|void
name|endProtected
parameter_list|()
function_decl|;
