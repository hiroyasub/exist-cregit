...
comment|//	LOG.debug("authenticating '"+username+"' with realm '"+realm.getId()+"'...");
try|try
block|{
return|return
name|realm
...
argument_list|()
decl_stmt|;
comment|//        A new_account = registeredRealm.instantiateAccount(registeredRealm, id, account);
name|AccountImpl
name|new_account
init|=
operator|new
...
condition|)
block|{
comment|//XXX: deleted realm's objects
comment|//				Configuration conf = Configurator.parse(document);
comment|//				if (isAccount) {
comment|//	            	Integer id = conf.getPropertyInteger("id");
comment|//	            	if (id != null&& !getSecurityManager().hasUser(id)) {
comment|//						AccountImpl account = new AccountImpl( null, conf );
comment|//	            		account.removed = true;
comment|//		            	addUser(account.getId(), account);
comment|//	            	}
comment|//				} else if (isGroup) {
comment|//	            	Integer id = conf.getPropertyInteger("id");
comment|//	            	if (id != null&& !getSecurityManager().hasGroup(id)) {
comment|//	                    GroupImpl group = new GroupImpl(null, conf);
comment|//	                    group.removed = true;
comment|//	            		addGroup(group.getId(), group);
comment|//	            	}
comment|//				}
block|}
else|else
