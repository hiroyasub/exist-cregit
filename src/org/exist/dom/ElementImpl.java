...
argument_list|)
expr_stmt|;
comment|//    	if(duplicateAttrs != null) {
comment|//    		removeAppendAttributes(transaction, duplicateAttrs, attribs);
comment|//    	} else {
comment|//    		NodeImplRef last = new NodeImplRef(this);
comment|//            final DocumentImpl owner = (DocumentImpl)getOwnerDocument();
comment|//	        if (children == 0) {
comment|//	            // no children: append a new child
comment|//	            appendChildren(transaction, nodeId.newChild(), -1, last, getPath(), attribs, true);
comment|//	        }
comment|//	        else {
comment|//	        	NamedNodeMap attrs = getAttributes();
comment|//                final StoredNode lastAttrib = (StoredNode) attrs.item(attrs.getLength() - 1);
comment|//	            if (lastAttrib == null || attrs.getLength() == children) {
comment|//                    appendChildren(transaction, null, firstChildID() + 1, last, getPath(), attribs, true);
comment|//	            } else {
comment|//                   	last.setNode(lastAttrib);
comment|//                    appendChildren(transaction, null, lastChildID() + 1, last, getPath(), attribs, true);
comment|//	            }
comment|//	        }
comment|//	        attributes += attribs.getLength();
comment|//            getBroker().updateNode(transaction, this);
comment|//            getBroker().reindexXMLResource(transaction, owner, owner, null);
comment|//    	}
block|}
specifier|private
name|NodeList
