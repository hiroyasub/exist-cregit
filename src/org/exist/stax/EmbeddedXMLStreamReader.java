...
name|broker
decl_stmt|;
comment|/**      * Construct an EmbeddedXMLStreamReader.      *      * @param doc the document to which the start node belongs.      * @param iterator a RawNodeIterator positioned on the start node.      * @param originNode an optional StoredNode whose nodeId should match the first node in the stream      *     (or null if no need to check); at most one of originNode and originProxy should be set      * @param originProxy an optional NodeProxy whose nodeId should match the first node in the stream      *     (or null if no need to check); at most one of originNode and originProxy should be set      * @param reportAttributes if set to true, attributes will be reported as top-level events.      * @throws XMLStreamException      */
specifier|public
name|EmbeddedXMLStreamReader
parameter_list|(
name|DBBroker
...
name|broker
expr_stmt|;
comment|// Seeking to a proxy with unknown address will reuse this reader, so do it before setting all
comment|// the fields otherwise they could get overwritten.
name|iterator
operator|.
name|seek
