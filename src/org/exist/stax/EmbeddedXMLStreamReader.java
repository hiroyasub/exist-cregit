...
argument_list|)
decl_stmt|;
comment|// states outside the range of events defined in {@link XMLStreamConstants}
specifier|private
specifier|static
name|int
name|BEFORE
...
literal|2
decl_stmt|;
comment|// members which don't (generally) change!
specifier|private
name|DBBroker
name|broker
decl_stmt|;
...
decl_stmt|;
comment|// mutable members which hold the current state of the stream
specifier|private
name|int
name|state
init|=
...
literal|false
expr_stmt|;
comment|// mark that we have a new state available
comment|// NOTE: this intentionally prevents END_DOCUMENT event being returned from {@link #next()}.
if|if
condition|(
name|state
operator|==
...
literal|true
expr_stmt|;
comment|// mark that we have consumed the current state
return|return
name|state
return|;
block|}
