...
begin_comment
comment|/**  * Class which encapsulates the DOMFile implementation.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/* replaced by removeDocument(document);         NodeRef ref = new NodeRef(document.getDocId());         final IndexQuery idx = new IndexQuery(IndexQuery.TRUNC_RIGHT, ref);         new DOMTransaction(this, domDb) {             public Object start() {                 try {                     domDb.remove(idx, null);                     domDb.flush();                 } catch (BTreeException e) {                     LOG.warn("start() - " + "error while removing doc", e);                 } catch (DBException e) {                     LOG.warn("start() - " + "error while removing doc", e);                 } catch (IOException e) {                     LOG.warn("start() - " + "error while removing doc", e);                 } catch (TerminatedException e) {                     LOG.warn("method terminated", e);                 }                 return null;             }         }         .run();         */
block|}
specifier|public
name|void
name|n8
...
comment|// first part can be replaced by removeBinaryResource and by n7
specifier|public
name|void
name|n9
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|/* replaced by dropIndex         new DOMTransaction(this, domDb, Lock.WRITE_LOCK) {             public Object start() {                 try {                     Value ref = new NodeRef(doc.getDocId());                     IndexQuery query =                         new IndexQuery(IndexQuery.TRUNC_RIGHT, ref);                     domDb.remove(query, null);                     domDb.flush();                 } catch (BTreeException e) {                     LOG.warn("btree error while removing document", e);                 } catch (DBException e) {                     LOG.warn("db error while removing document", e);                 } catch (IOException e) {                     LOG.warn("io error while removing document", e);                 } catch (TerminatedException e) {                     LOG.warn("method terminated", e);                 }                 return null;             }         }         .run();         */
block|}
specifier|public
name|void
name|dropIndex
