...
literal|true
decl_stmt|;
comment|/**      * Set to true if this database instance is running in read-only mode.      */
specifier|private
name|boolean
name|isReadOnly
init|=
literal|false
decl_stmt|;
comment|/**      * Is this database instance transactional?      */
specifier|private
name|boolean
...
literal|null
decl_stmt|;
comment|/**      * The global manager for access to collection configuration files.      *       * @uml.associationEnd multiplicity="(1 1)"      */
specifier|private
name|CollectionConfigurationManager
...
literal|null
decl_stmt|;
comment|/**      * Global transaction management.      *       * @uml.associationEnd multiplicity="(1 1)"      */
specifier|private
name|TransactionManager
name|txnManager
init|=
...
comment|//                collectionsCache = new CollectionCache(this, COLLECTION_BUFFER_SIZE, 20);
comment|//                xmlReaderPool = new XMLReaderPool(new XMLReaderObjectFactory(this), 5, 0);
name|syncDaemon
operator|=
operator|new
name|SyncDaemon
...
return|;
block|}
comment|/**      * Does this database instance support transactions?      *       * @return      */
specifier|public
name|boolean
name|isTransactional
parameter_list|()
...
argument_list|)
expr_stmt|;
comment|// set flag to indicate that we are initializing
name|initializing
operator|=
literal|true
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|// initialize transaction management
name|txnManager
operator|=
operator|new
name|TransactionManager
...
argument_list|)
expr_stmt|;
comment|// jmv collectionsCache = new CollectionCache(this, COLLECTION_BUFFER_SIZE, 20);
if|if
condition|(
operator|!
name|recovered
...
expr_stmt|;
block|}
comment|// process any waiting system tasks
...
condition|)
block|{
comment|// pool is idle: execute the task immediately
name|DBBroker
name|broker
init|=
operator|(
...
name|debug
argument_list|(
literal|"Scheduling system task: "
operator|+
name|taskConf
operator|.
name|getClassName
...
name|EXistException
argument_list|(
literal|"Failed to register system task: "
operator|+
name|e
operator|.
name|getMessage
