...
init|)
block|{
comment|//recheck here because now under collectionsDbLock
if|if
condition|(
name|isSubCollection
argument_list|(
...
expr_stmt|;
block|}
comment|// inherit the group to the destCollection if destination is setGid
if|if
condition|(
name|destination
operator|.
...
return|;
block|}
comment|/**      * Copies just the mode and ACL from the src to the dest      */
specifier|private
name|void
name|copyModeAndAcl
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|// NOTE: we already have a WRITE lock on childCollection
specifier|final
name|List
argument_list|<
name|ManagedCollectionLock
