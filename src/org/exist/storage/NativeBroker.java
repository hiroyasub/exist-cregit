...
expr_stmt|;
block|}
comment|// TODO(AR) The code below seems somewhat different to that in NativeBroker#copyResource presumably should be the same?
specifier|final
name|XmldbURI
...
comment|//                    moveCollectionBinaries(transaction, fsSourceDir, targetCollection, newName);
block|}
name|trigger
operator|.
...
return|;
block|}
comment|/**      * Moves only the binaries in {@code sourceDir} to {@code targetDir},      * there is no recursion into sub-directories.      *      * @param transaction The current transaction      * @param sourceDir The source directory (containing the binary objects) which is to be moved      * @param targetCollectionUri The target Collection which the files in the source collection are to be moved to      */
specifier|private
name|void
name|moveCollectionBinaries
parameter_list|(
specifier|final
name|Txn
name|transaction
...
argument_list|)
decl_stmt|;
comment|// iterate each file in the sourceDir
for|for
control|(
specifier|final
name|Path
...
argument_list|()
decl_stmt|;
comment|// note a backup target for the WAL
specifier|final
name|MessageDigest
name|targetFileMessageDigest
decl_stmt|;
specifier|final
name|Path
name|targetBackupFile
decl_stmt|;
if|if
condition|(
name|Files
...
condition|)
block|{
comment|// digest for the target file
name|streamableDigest
operator|.
name|reset
argument_list|()
...
expr_stmt|;
block|}
comment|// backup the target file, needed for undoing the transaction if recovery happens
if|if
condition|(
name|targetBackupFile
operator|!=
...
block|}
block|}
comment|// perform the actual binary file operation
comment|// move the binary file/directory to the new location on disk
name|Files
operator|.
name|createDirectories
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// fsSourceDir must be known in advance, because once the collection is moved
comment|// below, both collection and destination can point to the same resource
specifier|final
name|Path
name|fsSourceDir
init|=
...
expr_stmt|;
block|}
comment|// move the binaries in the collection
name|moveCollectionBinaries
argument_list|(
name|transaction
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// if there was a fs folder for holding binaries for the collection remove the (now empty folder) from disk
specifier|final
name|Path
name|fsBinCollection
init|=
...
name|error
argument_list|(
literal|"Unable to remove non-empty fs folder for Collection binaries: "
operator|+
name|fsBinCollection
operator|.
name|toAbsolutePath
...
argument_list|()
decl_stmt|;
comment|// make a copy for the WAL
specifier|final
name|Path
name|walCurrentValue
init|=
name|getCollectionFile
argument_list|(
name|fsJournalDir
...
specifier|final
name|MessageDigest
name|walCurrentValueMessageDigest
init|=
name|streamableDigest
operator|.
name|copyMessageDigest
...
block|}
comment|// perform the actual binary file operation
comment|// delete the binary file from disk
name|Files
operator|.
name|delete
argument_list|(
...
argument_list|()
decl_stmt|;
comment|// we must write a copy of the new binary file to disk first, so it can be redone or undone if the transaction needs to be recovered
specifier|final
name|Path
name|walNewValue
init|=
...
specifier|final
name|MessageDigest
name|currentValueMessageDigest
init|=
name|streamableDigest
operator|.
name|copyMessageDigest
...
block|}
comment|// perform the actual binary file operation
comment|// copy the binary file to the real location on disk
name|Files
operator|.
name|copy
argument_list|(
...
block|{
comment|//no journal, just do the binary operation
name|Files
operator|.
name|deleteIfExists
argument_list|(
...
specifier|final
name|MessageDigest
name|fsSourceDocumentMessageDigest
init|=
name|streamableDigest
operator|.
name|copyMessageDigest
...
specifier|final
name|MessageDigest
name|fsDestinationDocumentMessageDigest
decl_stmt|;
specifier|final
name|Path
name|fsDestinationDocumentBackup
decl_stmt|;
if|if
condition|(
name|Files
...
condition|)
block|{
comment|// digest for the destination file
name|streamableDigest
operator|.
name|reset
argument_list|()
...
argument_list|()
expr_stmt|;
comment|// destination backup file
name|fsDestinationDocumentBackup
operator|=
name|getCollectionFile
argument_list|(
...
block|}
comment|// perform the actual binary file operation
comment|// move the binary file to the new location on disk
name|Files
operator|.
name|createDirectories
argument_list|(
...
block|}
block|}
comment|// perform the actual binary file operation, delete the file!
name|Files
operator|.
name|delete
argument_list|(
