...
name|getProperty
argument_list|(
literal|"db-connection.collection-cache"
argument_list|)
expr_stmt|;
if|if
condition|(
...
comment|//is = new VariableByteInputStream(data);
name|is
operator|=
operator|new
name|VariableByteInputStream
...
condition|)
block|{
comment|//val = collectionsDb.get(key);
name|dis
operator|=
name|collectionsDb
operator|.
...
block|{
comment|//val = collectionsDb.get(addr);
name|dis
operator|=
name|collectionsDb
operator|.
...
literal|null
return|;
comment|//final byte[] data = val.getData();
name|VariableByteInputStream
name|istream
init|=
operator|new
...
argument_list|()
expr_stmt|;
comment|//collections.remove(collection);
block|}
block|}
...
argument_list|()
expr_stmt|;
comment|//			if (domIterator == null)
comment|//				domIterator = getNodeIterator(p);
comment|//			else
comment|//				 ((NodeIterator) domIterator).setTo(p);
comment|//			buf.setLength(0);
comment|//			getNodeValue(buf, domIterator, true);
comment|//			content = buf.toString();
try|try
block|{
name|domDb
operator|.
