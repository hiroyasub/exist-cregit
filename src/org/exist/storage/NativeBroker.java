...
throw|;
block|}
comment|// READ_LOCK the sourceCollection
try|try
init|(
specifier|final
name|ManagedCollectionLock
name|sourceCollectionLock
init|=
name|readLockCollection
argument_list|(
literal|null
...
init|)
block|{
comment|// WRITE LOCK the destinationCollection and its parent (targetCollection)
try|try
init|(
specifier|final
name|ManagedCollectionLock
name|destinationCollectionLock
init|=
name|writeLockCollection
argument_list|(
literal|null
...
init|)
block|{
comment|/*                  * At this point this thread should hold:                  *   READ_LOCKS on:                  *     1) sourceCollection                  *                  *   WRITE_LOCKs on:                  *     2) targetCollection                  *     3) destinationCollection                  */
comment|// we now pessimistically READ_LOCK the sourceCollection descendants
specifier|final
name|List
argument_list|<
name|ManagedCollectionLock
argument_list|>
name|sourceCollectionDescendantLocks
init|=
name|readLockDescendants
argument_list|(
name|sourceCollection
argument_list|,
name|sourceCollectionLock
...
try|try
block|{
comment|// we now pessimistically WRITE_LOCK the destinationCollection descendants that we are about to create
specifier|final
name|List
argument_list|<
name|XmldbURI
argument_list|>
name|destinationCollectionDescendentUris
init|=
name|sourceCollectionDescendantLocks
operator|.
name|stream
...
name|ManagedCollectionLock
argument_list|>
name|destinationCollectionDescendantLocks
init|=
name|writeLockCollections
argument_list|(
name|destinationCollectionDescendentUris
argument_list|,
name|destinationCollectionLock
...
block|{
comment|// iterate over the list in reverse order (to ensure correct order of release)
for|for
control|(
name|int
name|i
...
init|)
block|{
comment|// NOTE: we already have a READ lock on child
if|if
condition|(
name|child
operator|==
...
expr_stmt|;
block|}
comment|// TODO(AR) The code below seems quite different to that in NativeBroker#copyResource presumably should be the same?
specifier|final
name|XmldbURI
name|newDocName
init|=
...
throw|;
block|}
comment|// WRITE LOCK the sourceCollection and its parent
try|try
init|(
specifier|final
name|ManagedCollectionLock
...
argument_list|)
decl_stmt|;
comment|// NOTE: we already have a WRITE lock on sourceCollectionParent (in sourceCollectionLock)
if|if
condition|(
operator|!
name|sourceCollectionParent
...
throw|;
block|}
comment|// WRITE LOCK the destinationCollection
try|try
init|(
specifier|final
name|ManagedCollectionLock
...
init|)
block|{
comment|// NOTE: we already have a WRITE lock on destinationCollection (in destinationCollectionLock)
if|if
condition|(
name|existingDestinationCollection
operator|!=
...
block|}
block|}
comment|/*                  * At this point this thread should hold WRITE_LOCKs on:                  *   1) parent of sourceCollection                  *   2) sourceCollection                  *   3) targetCollection                  *   4) destinationCollection                  */
comment|// we now pessimistically WRITE_LOCK the sourceCollection descendants
specifier|final
name|List
argument_list|<
name|ManagedCollectionLock
...
block|}
block|}
comment|/**      * Acquires WRITE_LOCKs on multiple Collections      *      * Attempts to optimize the acquisition of locks by calling      * {@link LockManager#acquireCollectionWriteLock(ManagedCollectionLock, XmldbURI)} instead of      * {@link LockManager#acquireCollectionWriteLock(XmldbURI, boolean)}      *      * @param collectionUris A list of Collection URIs expected to be sorting top-down left-to-right      * @param parentCollectionLock An optional parentCollectionLock for the first collectionUri      *      * @return A list of WRITE_LOCKs in the same order as collectionUris      */
specifier|private
name|List
argument_list|<
name|ManagedCollectionLock
...
return|;
block|}
comment|/**      * Acquires WRITE_LOCKs on all descendant Collections of a specific Collection      *      * Locks are acquired in a top-down, left-to-right order      *      * Attempts to optimize the acquisition of locks by calling      * {@link LockManager#acquireCollectionWriteLock(ManagedCollectionLock, XmldbURI)} instead of      * {@link LockManager#acquireCollectionWriteLock(XmldbURI, boolean)}      *      * NOTE: It is assumed that the caller holds a {@link LockMode#WRITE_LOCK} on the      *     `collection`      *      * @param collection The Collection whose descendant WRITE_LOCKs should be acquired      * @param collectionLock An existing WRITE_LOCK acquired on `collection`      *      * @return A list of WRITE_LOCKs in the same order as collectionUris. Note that these should be released in reverse      *     order      */
specifier|private
name|List
argument_list|<
name|ManagedCollectionLock
...
return|;
block|}
comment|/**      * Acquires READ_LOCKs on all descendant Collections of a specific Collection      *      * Locks are acquired in a top-down, left-to-right order      *      * Attempts to optimize the acquisition of locks by calling      * {@link LockManager#acquireCollectionReadLock(ManagedCollectionLock, XmldbURI)} instead of      * {@link LockManager#acquireCollectionReadLock(XmldbURI)}      *      * NOTE: It is assumed that the caller holds a {@link LockMode#READ_LOCK} on the      *     `collection`      *      * @param collection The Collection whose descendant READ_LOCKs should be acquired      * @param collectionLock An existing READ_LOCK acquired on `collection`      *      * @return A list of READ_LOCKs in the same order as collectionUris. Note that these should be released in reverse      *     order      */
specifier|private
name|List
argument_list|<
name|ManagedCollectionLock
...
argument_list|)
decl_stmt|;
comment|// NOTE: we already have a lock on collection
while|while
condition|(
name|it
operator|.
...
argument_list|)
decl_stmt|;
comment|// NOTE: we already have a lock on childCollection
specifier|final
name|List
argument_list|<
name|ManagedCollectionLock
...
argument_list|)
expr_stmt|;
comment|// WRITE LOCK the collection and its parent
try|try
init|(
specifier|final
name|ManagedCollectionLock
...
argument_list|)
decl_stmt|;
comment|// NOTE: we already have a WRITE lock on the parent as part of collectionLock
...
comment|// 7) remove the documents nodes of the Collection from dom.dbx
...
expr_stmt|;
comment|//TODO(AR) this can be executed asynchronously as a task, we need to await the completion before unlocking the collection
comment|//TODO(AR) could optimise by only calling at the highest level (i.e. the first call to _removeCollection)
comment|// 8) remove any binary files that were in the Collection
name|removeCollectionBinaries
argument_list|(
name|transaction
argument_list|,
name|collection
...
argument_list|)
expr_stmt|;
comment|//TODO(DS) log first, rename second ???
comment|//TODO(DW) not sure a Fatal is required here. Copy and delete maybe?
name|Files
operator|.
name|move
argument_list|(
...
return|;
block|}
comment|/**      * Acquires a write lock on a Collection      *      * @param parentCollectionLock lock that is already held on the parent Collection or null      * @param collectionUri The uri of the collection to lock      * @param lockParent true if we should also write lock the parent Collection, only      *     relevant when {@code parentCollectionLock == null}      *      * @return A managed lock for the Collection      */
specifier|private
name|ManagedCollectionLock
name|writeLockCollection
parameter_list|(
...
block|}
block|}
comment|/**      * Acquires a READ lock on a Collection      *      * @param parentCollectionLock lock that is already held on the parent Collection or null      * @param collectionUri The uri of the collection to lock      *      * @return A managed lock for the Collection      */
specifier|private
name|ManagedCollectionLock
name|readLockCollection
parameter_list|(
...
name|error
argument_list|(
literal|"LockException while reindexing documents of collection '"
operator|+
name|collection
operator|.
name|getURI
...
name|error
argument_list|(
literal|"LockException while removing index of collection '"
operator|+
name|collection
operator|.
name|getURI
...
comment|// if we opened the Collection rather than creating it, obtain a lock in the transaction
if|if
condition|(
operator|!
name|created
...
block|}
block|}
comment|//TODO(AR) consider just using Files.createDirectories to create the entire path in one atomic go
specifier|final
name|XmldbURI
index|[]
name|segments
...
expr_stmt|;
block|}
comment|/**      * @param transaction      * @param blob The binary document to store      * @param fWriteData A function that given the destination path, writes the document data to that path      */
specifier|private
name|void
name|storeBinaryResource
parameter_list|(
