...
block|}
block|}
comment|//                    result.setSorted(storedDocument, ordered == ENTRIES_ORDERED);
if|if
condition|(
name|lastAncestor
operator|!=
...
parameter_list|)
block|{
comment|//        short nodeType = getIndexType(type);
specifier|final
name|NewArrayNodeSet
name|result
init|=
...
literal|true
decl_stmt|;
comment|//        boolean descendantAxis = selector instanceof DescendantSelector;
for|for
control|(
name|Iterator
argument_list|<
...
condition|)
block|{
comment|//                            long address = StorageAddress.read(is);
comment|//                            NodeProxy storedNode = new NodeProxy(storedDocument, nodeId, nodeType, address);
comment|//                            result.add(storedNode, gidsCount);
return|return
literal|true
return|;
block|}
...
block|{
comment|//                                long address = StorageAddress.read(is);
comment|//                                storedNode.setInternalAddress(address);
comment|//                                storedNode.setNodeType(nodeType);
comment|//                                result.add(storedNode, gidsCount);
return|return
literal|true
return|;
...
argument_list|)
expr_stmt|;
comment|//                    result.setSorted(storedDocument, ordered == ENTRIES_ORDERED&& !descendantAxis);
block|}
block|}
catch|catch
parameter_list|(
...
comment|//                                    storedNode.deepCopyContext(ancestor, contextId);
comment|//                                } else
comment|//                                    storedNode.copyContext(ancestor);
comment|//                                storedNode.addMatches(ancestor);
block|}
name|prevPosition
operator|=
operator|(
...
block|}
comment|//        LOG.debug("Found: " + result.getLength() + " for " + qname);
comment|//        if (sameDocSet) {
comment|//        	result.setDocumentSet(docs);
comment|//        }
return|return
literal|false
return|;
