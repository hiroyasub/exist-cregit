...
name|broker
decl_stmt|;
comment|/**      * @param broker DBBroker to use for compilation and execution      */
specifier|public
name|XQuery
parameter_list|(
specifier|final
...
condition|)
block|{
comment|//just setGid, so we use the current user as the effective user
name|effectiveSubject
operator|=
operator|new
name|EffectiveSubject
...
argument_list|)
expr_stmt|;
comment|//switch to effective user (e.g. setuid/setgid)
block|}
name|context
operator|.
name|getProfiler
...
argument_list|)
expr_stmt|;
comment|// track query stats before context is reset
name|broker
operator|.
name|getBrokerPool
argument_list|()
