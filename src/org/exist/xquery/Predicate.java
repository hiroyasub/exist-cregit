...
comment|/*             ValueSequence result = new ValueSequence(); 			Sequence innerSeq = inner.eval(contextSequence);			 			for(SequenceIterator i = innerSeq.iterate(); i.hasNext(); ) { 				NumericValue v = (NumericValue)i.nextItem().convertTo(Type.NUMBER); 				int pos = v.getInt() - 1;                 //Other positions are ignored     				if(pos>= 0&& pos< contextSequence.getLength()) 					result.add(contextSequence.itemAt(pos));                             			} 			return result;             */
return|return
name|evalBoolean
argument_list|(
name|contextSequence
