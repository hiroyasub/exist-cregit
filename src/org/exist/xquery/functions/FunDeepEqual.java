...
begin_comment
comment|/**  * Implements the fn:deep-equal library function.  * @linkplain http://www.w3.org/TR/xpath-functions/#func-deep-equal  *   * @author<a href="mailto:piotr@ideanest.com">Piotr Kaminski</a>  */
end_comment
...
argument_list|)
decl_stmt|;
comment|// if at least one is atomic type
if|if
condition|(
name|aSubTypeOfATOMIC
operator|||
...
condition|)
block|{
comment|// if one ist not atomic type
if|if
condition|(
operator|!
name|aSubTypeOfATOMIC
...
literal|false
return|;
comment|// both are atomic: call the algo. that is being used for the eq operator
try|try
block|{
return|return
name|GeneralComparison
...
return|;
block|}
comment|// the eq operator throws an exception if types are not comparable, deep-equal must not
catch|catch
parameter_list|(
name|XPathException
name|e
...
block|{
comment|// both are not atomic types
