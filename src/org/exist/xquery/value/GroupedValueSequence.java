...
begin_comment
comment|/**   * A sequence that containts items of one group specified by the group specs of   * an "group by" clause. Used by    * {@link org.exist.xquery.value.GroupedValueSequenceList}.   *    * This class is based on {@link org.exist.xquery.value.OrderedValueSequence}.   *    * WARNING : don't use except for group by clause    *    * @author Boris Verhaegen   */
end_comment
...
literal|0
decl_stmt|;
comment|//grouping keys values of this group
specifier|private
name|GroupSpec
name|groupSpecs
index|[]
...
return|return
operator|new
name|GroupedValueSequenceIterator
argument_list|()
return|;
block|}
...
return|;
block|}
comment|/**       * Check equality with self grouping keys values. Returns a boolean.       *        *  @param     otherGroupKey    An<code>Item[]</code> to compare with       *                          self grouping keys values       *  @return<code>true</code> if equal,<code>false</code>       *                          otherwise.           */
specifier|public
name|boolean
name|checkKeys
parameter_list|(
...
name|XPathException
block|{
comment|//TODO : possible performance gap here ! getLength() is costly
if|if
condition|(
name|this
operator|.
...
control|)
block|{
comment|// bv : compare atomic values using GeneralComparison
comment|//TODO : YES ! Well, valueComparison, which is a little bit more specific
if|if
condition|(
operator|!
name|GeneralComparison
...
name|XPathException
argument_list|(
literal|"expected a single value for group by expression "
operator|+
name|ExpressionDumper
operator|.
name|dump
