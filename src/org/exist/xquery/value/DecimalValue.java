...
argument_list|)
return|;
comment|/* 		if (Type.subTypeOf(other.getType(), Type.DECIMAL)) { 			try { 				return new IntegerValue(value.divide(((DecimalValue) other).value, 0, BigDecimal.ROUND_DOWN).toBigInteger(), Type.INTEGER); 			} catch (ArithmeticException e) { 				throw new XPathException("division by zero", e); 			} 		} 		throw new XPathException("idiv called with incompatible argument type: " + getType() + " vs " + other.getType()); 		*/
block|}
