...
block|}
block|}
comment|// at the end of the restore process, set the admin password to the
comment|// password restored from the backup. Up to here, we still used the old password.
name|Collection
name|root
init|=
name|DatabaseManager
...
argument_list|)
expr_stmt|;
comment|/* if we restored /db/system/users.xml, the admin password 					 * may have changed, so we will get a permission denied 					 * exception when continuing with the restore. We thus set 					 * the password back to the old one and change it to the  					 * new one at the very end of the restore. 					 */
if|if
condition|(
name|SecurityManager
operator|.
