...
name|name
operator|+
literal|"\" exists and update is not allowed"
argument_list|)
throw|;
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|//			if (oldDoc != null) {
comment|//				blob.setCreated(oldDoc.getCreated());
comment|//				blob.setLastModified(System.currentTimeMillis());
comment|//				blob.setPermissions(oldDoc.getPermissions());
comment|//
comment|//				LOG.debug("removing old document " + oldDoc.getFileName());
comment|//				if (oldDoc instanceof BinaryDocument)
comment|//					broker.removeBinaryResource((BinaryDocument) oldDoc);
comment|//				else
comment|//					broker.removeDocument(getName() + '/' + oldDoc.getFileName());
comment|//			} else {
comment|//				blob.setCreated(System.currentTimeMillis());
comment|//				blob.getPermissions().setOwner(broker.getUser());
comment|//				blob.getPermissions().setGroup(
comment|//						broker.getUser().getPrimaryGroup());
comment|//			}
name|manageDocumentInformation
argument_list|(
name|broker
